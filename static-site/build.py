...
def program_calendar_event(ev, col, row_by_slot):
    ...
    meta_html = (
        ...
        f'{program_event_meta(meta, bool(href))}</span>'
        if meta else ""
    )
    desc_html = f'<p>{esc(desc)}</p>' if desc else ""
    title_text = esc(title) if href else link_speaker_names(title)
...
def sec_speakers():
    ...
    cards = "".join(
        speaker_card(sp)
        for sp in d["list"]
        if sp.get("show_on_homepage", True)
    )
...
def page_speaker(sp):
  ...
  <div class="container">
    <nav class="breadcrumb speaker-back" aria-label="Návrat na seznam řečníků" style="justify-content:flex-start"><a href="/#speakers">‹ Zpět na řečníky</a></nav>
