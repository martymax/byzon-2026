...
| `CS-ADMIN-01` | dashboard, role, reservation cancel, session capacity, audit, organizační export a settings | `packages/domain/src/contracts/admin.ts` | `P9`, `F4-07`, `F4-08`, `F4-10` | `F4` | `partially integrated`: dashboard, role list/search/options + grant/revoke, reservation cancel/capacity a auditní keyset filtry mají produkční event-scoped porty. Export a settings pokračují v `AUX-13I,K`; všechny lokálně hotové routy čekají na společný staging `AUX-13A` gate. |
