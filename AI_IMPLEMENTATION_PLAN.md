...
| `CS-ADMIN-01` | dashboard, role, reservation cancel, session capacity, audit, organizační export a settings | `packages/domain/src/contracts/admin.ts` | `P9`, `F4-07`, `F4-08`, `F4-10` | `F4` | `partially integrated`: dashboard, role, rezervace/kapacity, audit a core settings mají produkční event-scoped porty. Skrytý support text je serverově unchanged-only do produktového rozhodnutí; export pokračuje v `AUX-13I` a lokálně hotové routy čekají na společný staging `AUX-13A` gate. |
