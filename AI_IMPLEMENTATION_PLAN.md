...
> Stav: implementační plán v6.43 – produkční integrace oznámení AUX-13G
>
...
| `SCOPE-2026-18` | Participant apply přijímá pouze `Uhrazeno`; ostatní a neznámé stavy nového účastníka neimportují. Pozdější downgrade existujícího účastníka zastaví pozvánku a jde k ruční kontrole bez automatického zrušení membershipu. Identita preferuje e-mail „prodej na jméno“, e-mail kupujícího je fallback jen pro objednávku s jednou způsobilou vstupenkou. | Skupinový nákup bez jednoznačných účastnických e-mailů nevytvoří duplicitní identity; neznámý status zastaví batch. Automatická suspendace čeká na praktické ověření refundu a nové schválení. | Rozhodnutí produktu 31. 8. 2026, [ADR-015](docs/adr/015-simpleshop-api-sync.md) |
...
| `CS-SUPPORT-01` | participant/ticket lookup a auditované support akce | `packages/domain/src/contracts/support.ts` | `P4-09`, `P9-03`, `F4-05` | `F4` | `partially integrated`: legacy ticket POST/no-store search a block/reactivate jsou current-event, rate-limited, reasoned/idempotentní a auditované. Importovaná identity resend/recovery čeká na `P4-06`–`P4-09`; transfer/storno se nevytváří. |
...
| `CS-ADMIN-01` | dashboard, role, reservation cancel, session capacity, audit, organizační export a settings | `packages/domain/src/contracts/admin.ts` | `P9`, `F4-07`, `F4-08`, `F4-10` | `F4` | `partially integrated`: dashboard, role, rezervace/kapacity, audit, export job list/download a core settings mají produkční event-scoped porty. Skrytý support text je serverově unchanged-only do produktového rozhodnutí; lokálně hotové routy čekají na společný staging `AUX-13A`, export navíc na storage gate `BLOCKER-INFRA-01`. |
...
**Závislost:** `BLOCKER-TKT-01` i `TKT-02` jsou uzavřené. Ticket credential,
claim a check-in vyřazuje
...
| BLOCKER-TKT-02 (uzavřen 31. 8. 2026) | Pouze `Uhrazeno` opravňuje k participant apply; downgrade jde k ruční kontrole. Identita preferuje e-mail „prodej na jméno“ a kupujícího použije jen pro jedinou způsobilou vstupenku v objednávce. | — | Organizátor | `P4-03` prod apply | Neznámý stav zastaví batch; skupinový nákup bez jednoznačných účastnických e-mailů nevytváří identity ani pozvánky. |
...
| 6.43 | 2. 9. 2026 | `AUX-13G` doplnil chybějící autorizovaný a event-scoped endpoint pojmenovaných announcement targets. Produkční workspace načítá options přes no-store safe-read port, při security failure maže draft a invaliduje shell a dovolí jen event nebo jednu serverem nabídnutou session. Immutable preview/send zachovává exact retry a kanonické `sent`/`already_sent` bez tvrzení o doručení; finální `[x]` čeká na staging auth/context E2E `AUX-13A`. |
...
| 6.48 | 2. 9. 2026 | `AUX-13B` připojil produkční `/admin` ke sdílenému dashboard workspace a nahradil ticket placeholdery skutečnými agregacemi importovaných účastníků, lokalizované importní dávky, sync publikace, kapacit po aktivitách a kritických oznámení. Worker stav je oddělený, raw chyby ani PII se nevracejí, sync failure/overbooking degradují a neřešitelná či nepovolená karta nevyrábí CTA. Unit, negativní permission/query testy a 1047/1047 browser scénářů jsou zelené; PostgreSQL endpoint regrese je připravená pro CI a finální `[x]` čeká na staging `AUX-13A`. |
| 6.49 | 2. 9. 2026 | `AUX-13H` připojil produkční `/admin/role` k bounded assignment listu, POST/no-store vyhledání aktivních ověřených členů s maskovaným kontaktem a serverem pojmenovaným station/session options. Current-event slug, `role:manage`, fáze a role-compatible scope jsou serverové guardy; moderator i Vedoucí aktivity zůstávají povinně session-scoped a archiv je read-only. Unit/contract a 1050/1050 browser scénářů jsou zelené, PostgreSQL permission/scope regrese je připravená pro CI; konkrétní personální obsazení zůstává `BLOCKER-OPS-01` a finální `[x]` čeká na staging `AUX-13A`. |
| 6.50 | 2. 9. 2026 | `AUX-13K` uzavřel lokální produkční core settings řez: read/update nyní kromě `event:settings:manage` ověřuje current-event slug, archivní zápis failuje zavřeně a exact retry/stale version/audit zůstávají kanonické. Neodsouhlasený `supportMessage` nelze změnit podvrženým requestem a jeho obsah se nekopíruje do audit diffu; UI jej dál nezobrazuje. Server/unit sada má 604 PASS a PostgreSQL regrese pro read, replay, stale, archive, permission a audit je připravená pro CI; finální `[x]` čeká na staging `AUX-13A`, support UI na `GAP-AUX-SETTINGS-01`. |
| 6.51 | 2. 9. 2026 | `AUX-13I` připojil `/admin/reporty` k autorizovanému bounded job listu se stabilním keyset cursorem a pravdivým mapováním queued/processing, ready, failed a expired. Ready-only download nově kromě export permission ověřuje current-event slug, po expiraci atomicky maže inline obsah a úspěch zapisuje do auditu; CSV neutralizuje i tab/CR control prefixy. Produkční workspace umí načíst další stránku a po queue obnoví historii; unit 604 PASS, worker 8/8 a browser 1053/1053 jsou zelené. Finální `[x]` čeká na storage-backed staging E2E po `BLOCKER-INFRA-01` a auth/context gate `AUX-13A`. |
