...
| `CS-ADMIN-01` | dashboard, role, reservation cancel, session capacity, audit, organizační export a settings | `packages/domain/src/contracts/admin.ts` | `P9`, `F4-07`, `F4-08`, `F4-10` | `F4` | `partially integrated`: dashboard má produkční event-scoped agregace a permission-safe UI, reservation cancel a samostatná session-level správa kapacity jsou live a audit má produkční keyset filtry. Role, export a settings pokračují podle vlastníků v `P9`/`AUX-13H,I,K`; staging dashboardu čeká na společný `AUX-13A` gate. |
