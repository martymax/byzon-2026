...
> Stav: `F0-01` dokončeno pro návrh Priority A
>
...
- 15 participant routes včetně stávajícího Priority A speaker/partner obsahu
  z `P3-04` a detailu minimálního in-app inboxu z `F2-05`;
- 15 organizačních a check-in routes;
- celkem 39 kanonických Priority A routes.
...
| --- | ----------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `D` | `draft`           | Veřejný marketingový rozcestník může fungovat, aktivace a participant obsah jsou zavřené. Admin připravuje event.        |
...
Offline mutace jsou výchozím stavem zakázané. Agenda add/remove a announcement
read state mohou dostat queue až po explicitním schválení v `CS-OFFLINE-01`.
Rezervace, ticket claim, support/admin mutace a check-in jsou online-only.
...
5. **Více** → přístupný sheet, nikoli další route

Sheet **Více** obsahuje Vstupenku, Praktické informace, Řečníky, Partnery,
Profil, Soukromí a Nastavení. `/app` má vždy viditelný ticket shortcut, takže
kritická vstupenka není závislá jen na menu. Aktivní stav má ikonu i label,
...
| -------------------- | -------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `/`                  | anonymní nebo libovolná relace; bez event permission           | `D/A/L/E/R`                                                           | Pokračovat správnou cestou → `Otevřít aplikaci`, `Aktivovat vstupenku` nebo `Přihlásit se` podle server bootstrapu | Kanonický root; Back opouští app. Nepoužít klientský redirect před bootstrapem.                                                     | `P0`; shell může být offline, stav relace se bez sítě nepředstírá                 | `PUB-R` + phase-specific CTA a bezpečný stav neznámé relace                              |
...
| `/app/agenda`                    | `participant`; `agenda:own:write` s `ownsResource`                                  | `A/L`; `E` read-only                               | Zobrazit vlastní plán → otevřít nejbližší session                                     | Deep link z bottom nav; Back `/app`. Zachovat den, scroll a canonical agenda version.                                      | agenda `P2` + program `P0`; owner-scoped cache, mutace jen po offline gate                  | `MEM-W` + prázdná agenda, conflict, pending add/remove, ended read-only                      |
| `/app/rezervace`                 | `participant`; `reservation:own:read`, mutace podle `CS-AGENDA-01`                  | `A/L`; `E` read-only                               | Spravovat rezervaci/waitlist → jedna action podle canonical state                     | Deep link z agenda/session; Back na zdroj nebo `/app/agenda`. Zachovat session context, ne stale confirm dialog.           | reservation/waitlist `P2`; čtení cache až po gate, mutace online-only                       | `MEM-W` + full/waiting/offered/expired/closed/stale-version a conflict                       |
...
| `/app/oznameni`                  | `participant`; plánované `announcement:own:read`; `announcementsEnabled`            | `A/L/E`; v `R` nedostupné                          | Přečíst provozní oznámení → otevřít nejnovější unread                                 | Deep link z bottom nav/badge; Back `/app`. Zachovat filtr unread/all a scroll.                                             | obsah `P1`, vlastní read state `P2`; cache podle `CS-ANN-01`, read queue až po offline gate | `MEM-W` + prázdný inbox, unread/read, removed announcement a pending read                    |
| `/app/oznameni/[announcementId]` | stejné jako inbox; server ověří audience membership                                 | `A/L/E`; `announcementsEnabled`                    | Přečíst oznámení → kontextový odkaz na dotčenou session                               | Přímý deep link z notifikace; Back obnoví inbox, fallback `/app/oznameni`. ID je opaque, ne audience údaj.                 | audience-scoped `P1/P2`; cache jen po owner gate                                            | `MEM-W` + audience denied jako bezpečné `404`, expired/removed a pending read                |
...
| Route              | Role a minimum                                                                                                 | Fáze/flag                                                                               | Primární úkol → dominantní CTA                                                     | Deep link, Back a zachování stavu                                                                                  | Data/offline/PII                                                             | UX profil                                                                                                |
| ------------------ | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `/admin`           | `organizer_admin`; plánované `operations:read`                                                                 | `D/A/L/E/R` podle retenčního minima                                                     | Najít provozní odchylku → jedna kontextová akce k nejzávažnějšímu stavu            | Kanonický admin home; Back opouští admin. Zachovat pouze bezpečný časový filtr.                                    | agregace `P1`, výjimky mohou být `P3`; online-only                           | `ADM-R` + žádná falešná live čísla, degraded worker/Redis/sync stav                                      |
| `/admin/obsah`     | `organizer_admin`; `program:manage`                                                                            | `D/A/L/E`; `R` read-only                                                                | Zkontrolovat obsah/publikaci → `Náhled publikace`                                  | Deep link z admin nav; Back `/admin`. Zachovat resource filtr, ne rozpracovaný stale publish confirm.              | draft/publikovaný obsah `P0/P1`; online-only admin write                     | `ADM-W` + draft empty, preview version, publish conflict/sync pending                                    |
| `/admin/program`   | `organizer_admin`; `program:manage`                                                                            | `D/A/L/E`; `R` read-only                                                                | Upravit program → `Přidat bod programu` nebo `Uložit změny` podle kontextu         | Deep link na session přes opaque ID; Back obnoví den/room/filter/scroll.                                           | draft program `P1`; online-only                                              | `ADM-W` + collision, timezone/day error, archived/cancelled session                                      |
| `/admin/recnici`   | `organizer_admin`; `program:manage`                                                                            | `D/A/L/E`; `R` read-only                                                                | Spravovat publikovaný profil → `Přidat řečníka`                                    | Deep link na interní ID; Back obnoví search/scroll. Jméno může být v obsahu, ne v query.                           | draft profil `P1`, veřejná část `P0`; online-only                            | `ADM-W` + duplicate slug, profile in use, archive confirmation                                           |
| `/admin/partneri`  | `organizer_admin`; `program:manage`                                                                            | `D/A/L/E`; `R` read-only                                                                | Spravovat partnery → `Přidat partnera`                                             | Deep link na interní ID; Back obnoví filtr/scroll.                                                                 | draft/published `P0/P1`; online-only                                         | `ADM-W` + duplicate slug, invalid URL, archive confirmation                                              |
| `/admin/vstupenky` | `organizer_admin`; plánované `ticket:any:manage` a pro import `CS-IMPORT-01`                                   | `D/A/L`; `E` read-only/support, `R` retenčně omezeno                                    | Zvalidovat a aplikovat import → aktuální krok upload/preview/confirm               | Deep link na opaque batch ID; Back mezi kroky bez raw řádků v URL. Stale preview se znovu načte.                   | import/support `P3`, raw kód `S`; online-only, raw file v private quarantine | `ADM-W` + unsupported file, validation conflicts, immutable preview, apply job/report                    |
| `/admin/ucastnici` | `organizer_admin`; plánované `participant:operational:read`, citlivá akce `ticket:any:manage`                  | `A/L/E`; `D` bez participant dat, `R` retenčně omezeno                                  | Najít účastníka a bezpečně vyřešit problém → `Vyhledat`; citlivá akce až v detailu | Deep link jen přes opaque interní ID; Back obnoví ne-PII filtr, jméno/e-mail neukládat do URL/history.             | provozní PII `P3`, ticket secret `S`; online-only                            | `ADM-W` + no result, ambiguous result, reason/confirmation/audit outcome                                 |
| `/admin/role`      | `organizer_admin`; plánované `role:manage`                                                                     | `D/A/L`; `E/R` revoke/read-only                                                         | Spravovat scoped operátorskou roli → `Přidat oprávnění`                            | Deep link na opaque assignment ID; Back `/admin`. Neobnovovat rozpracované grant/revoke confirm po session expiry. | role/scope metadata `P3`; online-only                                        | `ADM-W` + self-lockout prevention, last-admin guard, stale assignment, audit reason                      |
| `/admin/rezervace` | `organizer_admin`; `reservation:any:read`, override `agenda:any:override` s `auditedException`                 | `A/L`; `E/R` read-only                                                                  | Vyřešit kapacitní výjimku → kontextový `Provést override`                          | Deep link na session ID; Back obnoví day/session/filter. Reason se neukládá do URL ani po expiraci.                | rezervace a attendance `P3`; online-only                                     | `ADM-W` + full/waitlist/stale capacity, explicit impact, reason a audit                                  |
| `/admin/check-in`  | `organizer_admin`; `checkin:perform`, pro undo `checkin:undo`                                                  | `L`; `A` jen explicitní non-prod rehearsal, `E` read-only                               | Sledovat průběh/výjimky → `Otevřít odbavení` nebo vyřešit vybranou výjimku         | Deep link z dashboardu; Back `/admin`. Filtry bez PII lze zachovat.                                                | agregace `P1`, výjimky `P3`; online-only                                     | `ADM-W` + service degraded, duplicate/error list, undo reason/audit                                      |
...
| `/admin/reporty`   | `organizer_admin`; plánované `operations:read`, export `personal-data:operational:export` s `auditedException` | `A/L/E/R` dle retence                                                                   | Získat provozní souhrn → `Vytvořit export` jen po zvolení reportu                  | Deep link s allowlisted ne-PII filtrem; Back `/admin`. Expiring download se neobnovuje ze staré URL.               | agregace `P1`, export může být `P3`; online-only                             | `ADM-R` + report empty, async export queued/ready/expired/failed                                         |
| `/admin/audit`     | `organizer_admin`; plánované `audit:read`                                                                      | `D/A/L/E/R` dle retence                                                                 | Dohledat auditovanou změnu → `Použít filtry`                                       | Deep link jen s allowlisted action/time/opaque ID; žádné jméno/e-mail/reason v query. Back `/admin`.               | audit `P3`; online-only, citlivé before/after redacted                       | `ADM-R` + no result, redacted entry, retention boundary, request ID lookup                               |
| `/admin/nastaveni` | `organizer_admin`; plánované `event:settings:manage`                                                           | `D/A/L/E`; `R` read-only                                                                | Změnit minimální event nastavení → `Uložit změny`                                  | Deep link z admin nav; Back `/admin`. Neuložené změny potvrdit, stale version znovu načíst.                        | event config `P1`, role actor `P3`; online-only                              | `ADM-W` + invalid phase transition, stale version, destructive flag confirmation                         |
...
## 7. Permission gaps pro navazující kontrakty

Následující minimální permissions route mapa vyžaduje, ale aktuální
`packages/domain/src/permissions.ts` je zatím neobsahuje. Příslušný capability
owner je musí pojmenovat a doplnit s negativními event-scope testy. Do té doby
se route nesmí odemknout jen kontrolou role ani širším nesouvisejícím
oprávněním.
...
| vlastní announcement inbox/read          | `/app/oznameni*`                       | `CS-ANN-01`, `F2-05`, `P8-06`               |
| ticket support/manage                    | `/admin/vstupenky`, `/admin/ucastnici` | `CS-IMPORT-01`, `CS-SUPPORT-01`, `P4`, `P9` |
| minimální participant operational read   | `/admin/ucastnici`                     | `CS-SUPPORT-01`, `P9-03`                    |
| event-scoped role assignment manage      | `/admin/role`                          | `CS-ADMIN-01`, `P9-02`                      |
| operations/dashboard/report read         | `/admin`, `/admin/reporty`             | `CS-ADMIN-01`, `P9-01`, `P9-07`             |
| audit read                               | `/admin/audit`                         | `CS-ADMIN-01`, `P9-04`                      |
| event settings manage                    | `/admin/nastaveni`                     | `CS-ADMIN-01`, `P9-09`                      |

`checkin:own-code:read` je pro `/app/vstupenka` pouze současný nejbližší
permission. `CS-TICKET-01` musí při schema review potvrdit, zda pokrývá celý
ticket state/presentation scope, nebo zavést přesnější vlastní ticket
permission.
