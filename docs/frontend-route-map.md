...
> Stav k 16. 8. 2026: frontend je v6 scope-aligned a `UI ready (mocked)`;
> produkční integrace a UAT zůstávají otevřené
>
...
nesmějí vzniknout ani po Gate A bez nové změny scope.
...
| `/onboarding`        | ověřená identita a pending/active event vztah dle `CS-BOOT-01` | `A/L`; `E` jen dokončení dříve založeného vztahu, pokud server dovolí | Dokončit profil a právní acknowledgement → `Pokračovat`/`Dokončit`                            | Deep link po auth/claim; Back mezi dvěma kroky, exit potvrdit při neuložené změně. Po dokončení safe `returnTo` nebo `/app`.        | jméno/e-mail/dobrovolný telefon a acknowledgement `P2`; bez networking choice | `MEM-W` + validace E.164, chybějící právní verze a povinné acknowledgement               |
...
| `/app/agenda`                    | `participant`; `agenda:own:write`, `reservation:own:read`, mutace podle `CS-AGENDA-01`   | `A/L`; `E` read-only                               | Zobrazit vlastní plán a spravovat rezervaci/waitlist → jedna canonical session akce                 | Deep link z bottom nav; Back `/app`. Zachovat den, scroll, session context a canonical agenda version.                 | agenda/rezervace `P2` + program `P0`; owner-scoped cache, rezervace online-only        | `MEM-W` + empty/conflict/pending, full/waiting/promoted/closed/stale a ended read-only       |
...
Identitní pravidlo je jednotné: autorizovaný administrátor pracuje s dostupným
jménem a celým kontaktním e-mailem, nikdy se syntetickou maskovanou identitou.
Vedoucí aktivity a moderátor vidí u účastníků pouze jméno, bez e-mailu.

| Route              | Role a minimum                                                                                       | Fáze/flag                                                                               | Primární úkol → dominantní CTA                                                     | Deep link, Back a zachování stavu                                                                             | Data/offline/PII                                                           | UX profil                                                                                                          |
| ------------------ | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `/admin`           | `organizer_admin`; `operations:read`                                                                 | `D/A/L/E/R` podle retenčního minima                                                     | Najít provozní odchylku → jedna kontextová akce k nejzávažnějšímu stavu            | Kanonický admin home; Back opouští admin. Nepřijímá query filtr ani do URL neukládá agregované výjimky.       | event-scoped agregace `P1`; private/no-store, bez PII a raw worker chyb    | `ADM-R` + šest skutečných stavových karet, sync degraded a source-of-truth outbox stav bez falešné live telemetrie |
...
| `/admin/vstupenky` | `organizer_admin`; `ticket:any:manage` a `CS-IMPORT-01`                                              | `D/A/L`; `E` read-only/support, `R` retenčně omezeno                                    | Načíst, zkontrolovat a aplikovat SimpleShop import → aktuální krok preview/confirm | Deep link na opaque batch ID; Back mezi kroky bez raw řádků v URL. Stale preview se znovu načte.              | import/support `P3`, raw kód `S`; online-only, bez browserového souboru    | `ADM-W` + source/validation conflicts, immutable preview, exact apply/report                                       |
...
| `/admin/role`      | `organizer_admin`; `role:manage`                                                                     | `D/A/L`; `E` bez přidání člena, `R` read-only                                           | Spravovat členy, pozvánky a oprávnění → `Přidat člena`                             | Jméno ani e-mail se neukládají do URL. Back `/admin`; po expiry zahodit rozpracovaný modal a důvod změny.     | členové, e-mail a role `P3`; private/no-store, online-only                 | `ADM-W` + self/last-admin guard, current-event a role-compatible scope guard, stale version, audit reason          |
| `/admin/rezervace` | `organizer_admin`; `reservation:any:read`, override `agenda:any:override` s `auditedException`       | `A/L`; `E/R` read-only                                                                  | Vyřešit kapacitní nebo storno výjimku → kontextový auditovaný zásah                | Deep link na session ID; Back obnoví day/session/filter. Reason se neukládá do URL ani po expiraci.           | session-first keyset read, jméno a e-mail `P3`; online-only                | `ADM-W` + full/waitlist/stale capacity, cursor page, cancel/override, explicit impact, reason a audit              |
| `/admin/oznameni`  | `organizer_admin`; `announcement:send`; `announcementsEnabled`                                       | `A/L/E`; `R` read-only                                                                  | Připravit a potvrdit kritické event/session oznámení → `Náhled publika`/`Odeslat`  | Deep link na opaque draft/send ID; Back `/admin`. Po session expiry znovu načíst immutable preview.           | text `P1`, audience agregace `P1`, příjemci `P3`; online-only              | `ADM-W` + pouze critical, event/affected sessions, stale preview, duplicate send, job/DLQ stav                     |
| `/host/aktivity`   | `room_operator` („Vedoucí aktivity“); `reservation:assigned:read` s assigned session/room contextem  | `A/L/E`; online-only                                                                    | Přečíst jména, firmy a stav rezervace u vlastních aktivit → bez mutační CTA        | Přímý deep link po loginu; Back opouští host režim. Cizí session server vrací bezpečné forbidden/not-found.   | minimum rosteru jiných osob `P3`; no-store, bez telefonu/e-mailu/exportu   | `ADM-R` + empty assignment, session-scoped read, permission/session-expired; bez attendance write                  |
...
`reservation:assigned:read`. `CS-ANN-01` používá `announcement:own:read` s
recipient kontextem a send je pouze pro admina. Otevřená zůstává už jen úplná
ticket presentation autorita:
