...
| `/admin/role`      | `organizer_admin`; `role:manage`                                                                     | `D/A/L`; `E` jen revoke, `R` read-only                                                  | Spravovat scoped operátorskou roli → `Přidat oprávnění`                            | Bounded opaque cursor; PII hledání jen POST body. Back `/admin`; po expiry zahodit rozpracované potvrzení.    | role/scope metadata a maskovaný kontakt `P3`; private/no-store, online-only | `ADM-W` + current-event a role-compatible scope guard, stale assignment, audit reason                              |
| `/admin/rezervace` | `organizer_admin`; `reservation:any:read`, override `agenda:any:override` s `auditedException`       | `A/L`; `E/R` read-only                                                                  | Vyřešit kapacitní nebo storno výjimku → kontextový auditovaný zásah                | Deep link na session ID; Back obnoví day/session/filter. Reason se neukládá do URL ani po expiraci.           | session-first keyset read, maskovaná reference `P3`; online-only            | `ADM-W` + full/waitlist/stale capacity, cursor page, cancel/override, explicit impact, reason a audit              |
