...
## Aktualizace AQ-00 — 7. 9. 2026

[ADR-017](adr/017-participant-collaborators-and-private-question-follow-ups.md)
nově povoluje ruční participant creation jako primární cestu vedoucích a speakerů,
SimpleShop jako alternativu a soukromý follow-up linked speakerů. Pozvánka je
samostatná participant operace; veřejná registrace a check-in zůstávají mimo scope.
[Inventář](evidence/participant-access-live-qa-inventory.md) zachycuje 17 Q&A session,
kanonický networking na Leadership Stage a neověřené účty. Runtime, administrační průvodce, owner history, tablet feed, odpovědi a QR jsou implementované a lokálně testované. Skutečné účty a staging rehearsal zůstávají před zapnutím k doložení; postup obsahuje [runbook](runbooks/participant-access-live-qa.md).
...
| `SCOPE-2026-03` řečník bez obecného portálu             | `speaker` a linked profily; nový follow-up flag default OFF dle ADR-017       | `question:own-session:answer`; bez materials                                                         | `/host/dotazy`; bez `/speaker`                                                                            | implementováno a lokálně testováno; staging UAT otevřené                                                               |
...
| `SCOPE-2026-05` soukromé Q&A bez votes/polls/projection | `questions`, explicitní capability a `question_answers`                       | runtime submit/feed, owner read a speaker follow-up                                                  | `/app/interakce/[sessionId]` a `/moderator/[sessionId]` existují                                          | lifecycle integrován; fyzický staging rehearsal otevřený                                                               |
...
| `speaker`                            | doplňková role k participant baseline; follow-up po vlastní linked session   | materials, obecný portál, roster bez `room_operator`, identita autora |
...
- event flags `speaker_portal_enabled`, `polls_enabled`,
  `social_wall_enabled`.
