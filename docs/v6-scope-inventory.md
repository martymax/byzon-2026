...
kanonický networking na Leadership Stage a neověřené účty. Nový runtime zatím
není implementovaný ani ověřený na stagingu.
...
| `SCOPE-2026-03` řečník bez obecného portálu             | `speaker` a linked profily; nový follow-up flag default OFF dle ADR-017       | plánovaná `question:own-session:answer`; bez materials                                               | plánované `/host/dotazy`; bez `/speaker`                                                                  | scope aktualizován; AQ-01/SPEAKER-03/04 otevřené                                                                       |
...
| `SCOPE-2026-05` soukromé Q&A bez votes/polls/projection | `questions` existuje; plánované capability a `question_answers`               | existující submit/feed, plánovaný owner read a speaker follow-up                                     | `/app/interakce/[sessionId]` a `/moderator/[sessionId]` existují                                          | základ integrován; nový lifecycle a rehearsal otevřené                                                                 |
...
| `speaker`                            | doplňková role k participant baseline; plánovaný follow-up po vlastní linked session | materials, obecný portál, roster bez `room_operator`, identita autora |
