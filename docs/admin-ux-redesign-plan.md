...
| `AUX-05`             | `/admin/vstupenky`                                   | refaktoruje `F4-02` až `F4-04` | `ADR-015`, `CS-IMPORT-01`, integrovaný `P4-02` preview a `P4-03` apply; bez file uploadu       |
...
- Produkční `AdminImportWorkspace` podle `ADR-015` spouští serverové read-only
  načtení ze SimpleShop API, nenabízí CSV/XLSX upload a po lidské kontrole
  používá oddělený reasoned/idempotentní `P4-03` apply s výsledným reportem.
...
- `AdminContextResponse` od `AUX-02C` odděluje permission, stav feature
  oznámení a explicitní `canEnterCheckin`. Klient capability nikdy neodvozuje
  z role. Produkční server vrací pro baseline 2026 `canEnterCheckin: false`,
  protože odbavení zůstává mimo launch scope; role/flag fixtures ověřují i
  budoucí povolenou kombinaci.
...
check-in a notification vlastní `GAP-AUX-DASH-ACTION-01`; do jeho uzavření se
z metriky nesmí vyrobit nefunkční CTA. Explicitní context capability je od
`AUX-02C` dostupná, ale pro launch scope 2026 zůstává produkčně `false`.
...
- `BLOCKER-RES-03` uzavřelo `P5-05`: transfer nebo storno vstupenky ruší
  aktivní rezervace a uvolňuje kapacitu. Produkční napojení této ticket
  transition nadále vlastní `P4-09`; klient dopad nikdy neprovádí ani
  nedopočítá sám.
...
Produkční `P4-02` implementuje server-only, autorizovaný a rate-limitovaný
SimpleShop preview bez file uploadu. `P4-03`/`AUX-13D` přidává potvrzení a
report až po opětovném ověření stejného immutable snapshotu; historická file
větev smí zůstat pouze jako nezaměnitelná testovací fixture, ne jako paralelní
produkční cesta.
...
Produkční read je session-first a používá stabilní keyset cursor podle času
a ID aktivity. UI skládá načtené stránky bez duplicit, poctivě označuje souhrn
jako souhrn načtených aktivit a dovolí explicitně načíst další stránku.
Legacy reservation-level a capacity list endpoint zůstávají pouze pro rollout
kompatibilitu staršího klienta; nová obrazovka je nepoužívá.

Edge case rezervací při transferu nebo stornu vstupenky má rozhodnutí
`BLOCKER-RES-03` v hlavním plánu; jeho ticket-transition implementaci vlastní
`P4-09`, nikoli tato rezervační stránka.
...
vyhledání ověřené osoby posílá PII pouze POST/no-store body a options vracejí
jen serverem pojmenované kompatibilní stanoviště nebo session. Ruční UUID pole
není povolený interim UX.
...
| ---------------------------- | ---- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| `GAP-AUX-SHELL-01`           | [~]  | Produkční shell má context adapter a route gate; chybí staging E2E evidence všech rout                                 | `AUX-13A`, `P9` / 2026-09-02                                              | shell `integrated`                  | Lokální route/permission matice hotová; zavře deploy aktuálního main + přihlášený staging auth/context E2E |
| `GAP-AUX-CONTEXT-01`         | [x]  | Admin context neměl feature flags ani explicitní `canEnterCheckin`; check-in bootstrap má vlastní `confirm/undo` model | `AUX-02C` / 2026-09-01                                                    | —                                   | Context schema, server read, fixtures a role/flag test matice; 2026 server explicitně false                |
| `GAP-AUX-DASH-01`            | [x]  | Produkční metrics a bezpečný degraded worker/sync stav                                                                 | `AUX-13B`, `P9-01`, `P8-10`, `P6-06` / 2026-09-02                         | —                                   | Šest agregovaných ID, explicitní sync/full/overbooked stavy a oddělený no-error worker souhrn              |
| `GAP-AUX-DASH-ACTION-01`     | [x]  | Activation/check-in/notification metriky nemají vždy permission-safe řešitelný target                                  | `AUX-03B`, `AUX-13B`, `P9-01`, `P6-06`, `P8-10` / 2026-09-02              | —                                   | Bez serverem řešitelného targetu zůstává karta bez CTA; negativní permission/capability browser test       |
| `GAP-AUX-IMPORT-01`          | [x]  | Produkční SimpleShop preview, participant apply a report bez browserového souboru                                      | `AUX-13D`, `P4-03` / 2026-09-02                                           | —                                   | Transakční apply, source revalidation, exact retry, PII/credential regrese a browserový report             |
| `GAP-AUX-TICKET-01`          | [x]  | Lidský vyhledávač cílové vstupenky pro reassign/transfer                                                               | `AUX-06B`, `P9-03`, `P4-09` / 2026-09-02                                  | —                                   | Strict reference request, source version, no/single/ambiguous response, stale problem a fixtures           |
...
| `AUX-02A` | U0  | [x]  | UI            | Route-aware chrome bez veřejného headeru a vnořeného main na `/admin`              | `AUX-00B`, `AUX-01A`                                                                                                                        | —                                               | `AUX-01B`, `AUX-02C`                                                        | `ADR-013`                                                | N/A               | `route-aware-chrome`; SSR/unit landmark test                                                                               |
| `AUX-02B` | U0  | [x]  | architecture  | Oddělit shell view od preview/production adapteru a zachovat per-route fail-closed | `AUX-02A`                                                                                                                                   | —                                               | `AUX-02C`                                                                   | `CS-ADMIN-01`                                            | N/A               | normalizovaný view + context/transport adapter; forbidden child se nemountuje                                              |
| `AUX-02C` | U0  | [x]  | contract      | Rozšířit context/fixtures o explicitní feature a check-in capability               | `AUX-00B`                                                                                                                                   | —                                               | `AUX-02A`, `AUX-02B`                                                        | `GAP-AUX-CONTEXT-01`, `ADR-011`                          | contract ready    | schema/server/fixtures; role/flag matice; produkční 2026 capability=false                                                  |
| `AUX-02D` | U0  | [x]  | UI            | Seskupená permission/feature-aware navigace z §7                                   | `AUX-01B`, `AUX-02B`, `AUX-02C`                                                                                                             | —                                               | `AUX-02E`                                                                   | `CS-ADMIN-01`                                            | N/A               | grouped nav, `prefetch={false}`, permission/feature/capability testy                                                       |
...
| `AUX-04A` | U1  | [x]  | UI            | List-first obsahová IA, typ selector a lidské seskupení 8 zdrojů                   | `AUX-01B`, `AUX-03A`                                                                                                                        | —                                               | `AUX-03B`, `AUX-05A`, `AUX-06A`                                             | `P3-05`                                                  | N/A               | pět oblastí, osm zdrojů, allowlist `oblast`/`typ`, list-first desktop/mobile                                               |
| `AUX-04B` | U1  | [x]  | UI            | Přesné labely, auto adresa, speaker picker a přístupné řazení                      | `AUX-01C`, `AUX-04A`                                                                                                                        | —                                               | `AUX-04C`                                                                   | `P3-05`                                                  | N/A               | progressive adresa/pořadí, checkbox picker a přesné labely; browser 45/45                                                  |
| `AUX-04C` | U0  | [x]  | contract      | Human publication summary: názvy/dopad/revision/time/change count                  | `AUX-00B`                                                                                                                                   | —                                               | `AUX-04B`                                                                   | `GAP-AUX-CONTENT-01`, `P3-05`, `P3-06`, `P3-07`, `P3-08` | contract ready    | domain DTO + server diff + fixtures bez raw ID; domain 190/190                                                             |
| `AUX-04D` | U1  | [x]  | UI            | Lidský review/publish flow nad schváleným summary                                  | `AUX-04A`, `AUX-04C`                                                                                                                        | —                                               | —                                                                           | `P3-07`                                                  | N/A               | názvy a dopady, primary confirm, serverový čas; checksum jen v technical details                                           |
| `AUX-04E` | U1  | [x]  | QA            | Core obsah dirty/stale/conflict/archive/publish test matice                        | `AUX-04B`, `AUX-04D`                                                                                                                        | —                                               | `AUX-05C`, `AUX-06D`                                                        | `P3-07`                                                  | UI ready (mocked) | browser 939/939; unit 552/552; timezone, Back, archive a obě ambiguous větve                                               |
| `AUX-04F` | U2  | [x]  | contract      | Bezpečný asset read/resolver a picker/upload contract pro foto/logo                | `AUX-00B`                                                                                                                                   | —                                               | `AUX-04A`                                                                   | `ADR-007`, `P3-01`                                       | contract ready    | storage-independent domain schema, problémy a syntetické fixtures; `P3-13` zůstává otevřený                                |
| `AUX-04G` | U2  | [x]  | UI            | Asset placeholder/preview/replace/remove UI a testy bez raw asset ID               | `AUX-01B`, `AUX-04F`                                                                                                                        | —                                               | `AUX-04D`                                                                   | `AUX-04F`, `ADR-007`                                     | UI ready (mocked) | placeholder bez produkčního resolveru; preview/progress/error/remove/read-only testy                                       |
| `AUX-05A` | U1  | [x]  | UI            | Čtyřkroková aktualizace vstupenek: zdroj→kontrola→potvrzení→výsledek               | `AUX-01B`, `AUX-03A`                                                                                                                        | —                                               | `AUX-04A`, `AUX-06A`                                                        | `ADR-015`, `CS-IMPORT-01`, `P4-02`                       | N/A               | čtyři textové kroky, bezpečný Back a live SimpleShop preview bez browserového souboru                                      |
| `AUX-05B` | U1  | [x]  | UI            | Chyby první, záznamový souhrn a lidské potvrzení přesného dopadu                   | `AUX-01C`, `AUX-05A`                                                                                                                        | —                                               | —                                                                           | `CS-IMPORT-01`                                           | N/A               | combined problem filtr, desktop tabulka/mobile karty, primary confirm a lidský report                                      |
| `AUX-05C` | U1  | [x]  | QA            | Zdroj/bez změn/konflikt/neznámý/stale/ambiguous/offline matice                     | `AUX-05B`                                                                                                                                   | —                                               | `AUX-04E`, `AUX-06D`                                                        | `CS-IMPORT-01`                                           | UI ready (mocked) | browser 963/963; exact idempotency retry, stale reload, session wipe a server-only request boundary                        |
| `AUX-06A` | U1  | [x]  | UI            | Účastnické hledání, výsledky, detail a permission split                            | `AUX-01B`, `AUX-03A`                                                                                                                        | —                                               | `AUX-04A`, `AUX-05A`                                                        | `CS-SUPPORT-01`                                          | N/A               | POST/no-store bez URL PII; desktop tabulka, mobilní karty a read-only detail                                               |
...
| `AUX-12A` | U0  | [~]  | QA            | Cross-route axe, keyboard a screen-reader smoke                                    | `AUX-11B`                                                                                                                                   | —                                               | `AUX-12C`                                                                   | N/A                                                      | N/A               | 11 rout × 7 viewportů: axe, landmarks, heading order a keyboard zelené; fyzický VoiceOver/NVDA smoke čeká na UAT           |
| `AUX-12B` | U0  | [x]  | QA            | Visual/responsive 320–1440, 200% zoom a reduced motion                             | `AUX-12A`                                                                                                                                   | —                                               | `AUX-12C`                                                                   | N/A                                                      | N/A               | 320–1440, 1280×800, 200% reflow, reduced motion a 33 screenshotů; opraven mobilní souhrn rezervací                         |
| `AUX-12C` | U2  | [~]  | QA            | Bundle, max-page list performance a CLS audit                                      | `AUX-02G`, `AUX-03C`, `AUX-04E`, `AUX-05C`, `AUX-06D`, `AUX-07C`, `AUX-08C`, `AUX-09C`, `AUX-10G`                                           | —                                               | `AUX-11B`, `AUX-12A`                                                        | N/A                                                      | N/A               | technická akceptace hotová: max 100/100/50/500/5, CLS 0,03246 a long task 0 ms; `[x]` čeká na fyzický `AUX-12A`            |
| `AUX-12D` | U0  | [x]  | security-QA   | Cross-route privacy/security review mockované vrstvy                               | `AUX-12A`, `AUX-12B`                                                                                                                        | —                                               | `AUX-12C`                                                                   | N/A                                                      | N/A               | mock boundary, scope, P3 wipe, no-store, URL/telemetry a permission review v `docs/evidence/admin-ux-qa.md`                |
| `AUX-13A` | U0  | [~]  | integration   | Produkční shell/context a bezpečný per-route gate                                  | `AUX-02G`                                                                                                                                   | `GAP-AUX-SHELL-01`                              | `AUX-13C`                                                                   | `CS-ADMIN-01`, `GAP-AUX-SHELL-01`                        | integrated        | lokální shell/context/route/security matice hotová; staging je na starém release a chybí přihlášený UAT actor              |
| `AUX-13B` | U1  | [~]  | integration   | `/admin` dashboard                                                                 | `AUX-03C`, `AUX-13A`                                                                                                                        | —                                               | `AUX-13D`, `AUX-13E`                                                        | `P9-01`, `P6-06`, `P8-10`                                | integrated        | produkční workspace + agregovaný endpoint + permission/PII/browser regrese hotové; staging `AUX-13A` zbývá                 |
| `AUX-13C` | U1  | [~]  | integration   | `/admin/obsah` s jednotným shell a produkčním content portem                       | `AUX-04E`, `AUX-13A`                                                                                                                        | —                                               | `AUX-13B`                                                                   | `P3-05`, `P3-06`, `P3-07`, `P3-08`                       | integrated        | produkční wrapper používá jediný shell context/fetch port a invaliduje celý scope; staging `AUX-13A` zbývá                 |
| `AUX-13D` | U1  | [~]  | integration   | `/admin/vstupenky` SimpleShop preview→apply→report bez browserového souboru        | `AUX-05C`, `AUX-13A`, `P4-02`, `P4-03`                                                                                                      | —                                               | `AUX-13B`, `AUX-13E`                                                        | `ADR-015`, `GAP-AUX-IMPORT-01`, `CS-IMPORT-01`           | integrated        | source revalidation + transakční participant apply/report + exact retry hotové; staging `AUX-13A` zbývá                    |
| `AUX-13E` | U1  | [~]  | integration   | `/admin/ucastnici` search a support actions                                        | `AUX-06D`, `AUX-13A`, `P4-09`, `P9-03`                                                                                                      | `BLOCKER-AUTH-01`, `GAP-AUX-SUPPORT-ACTIONS-01` | `AUX-13B`, `AUX-13D`                                                        | `CS-SUPPORT-01`                                          | integrated        | legacy ticket search/block/reactivate auth, current-event, PII rate limit a audit hotové; imported identity resend zbývá   |
...
| `AUX-13J` | U1  | [~]  | integration   | `/admin/audit` query a cursor                                                      | `AUX-10G`, `AUX-13A`, `P9-04`                                                                                                               | —                                               | `AUX-13I`, `AUX-13K`                                                        | `CS-ADMIN-01`, `GAP-AUX-AUDIT-01`                        | integrated        | event/category/action/actor/outcome/time/request/cursor filtry, exhaustive registry a redigované DTO hotové; staging zbývá |
...
- [x] **02A-1:** Právě jeden skip link a jeden main v server-rendered i
      hydrated DOM; public chrome není v accessibility tree.
- [x] **02B-1:** Shell view je nezávislý na mocked/production portu a per-route
      fail-closed guard zůstává zachovaný.
- [x] **02C-1:** Context kontrakt rozlišuje permission, feature stav a
      `canEnterCheckin`; role/flag kombinace mají validované fixtures.
- [x] **02D-1:** Nav odpovídá §7, skrývá chybějící permission, vysvětluje
      feature-off a nepřednačítá P3 data.
- [x] **02E-1:** Event name, phase a actor label jsou lidské; UUID/timezone
      nejsou v shellu; menu má skutečné dostupné akce.
- [x] **02F-1:** Drawer splní dialog semantiku, inert, focus trap/restore,
      Escape, body lock a safe area.
- [x] **02G-1:** Každá route má metadata title, stabilní loading/error a právě
      jeden focus přesun po navigaci.
- [x] **02G-2:** 320 px, 200% zoom a keyboard focus nemají překrytí ani page
...
- [x] **04A-1:** Po otevření je první seznam, ne create formulář ani checksum;
      osm zdrojů a vnořené typy odpovídají §10.2 bez změny serverových typů.
- [x] **04B-1:** Každé pole má specifický label/helper; slug a pořadí jsou
      progressive; speaker picker funguje myší, dotykem i klávesnicí.
- [x] **04C-1:** Contract summary nese autoritativní změny, revision/time a
      title-level dopad, nebo explicitně označí hodnotu jako nedostupnou.
- [x] **04D-1:** Review ukazuje lidské názvy a dopad, ne raw session IDs;
      publikace používá primary confirm a archivace danger confirm. Permanentní
      delete není v baseline kontraktu ani UI.
- [x] **04E-1:** Archive, timezone, version, collision, dirty, stale a ambiguous
      invariants mají regresní testy celé routy.
- [x] **04E-2:** Produkční content port není nahrazen preview portem.
- [x] **04F-1:** Nový asset slice definuje autorizovaný event-scoped
      read/resolver, typ/velikost/purpose, preview, upload, replace/remove,
      problems a syntetické fixtures bez trvalé storage URL.
- [x] **04G-1:** Foto/logo UI nikdy nežádá raw asset ID; bez resolveru má
...
- [x] **05A-1:** Stepper jednoznačně rozlišuje serverové načtení ze SimpleShopu,
...
- [x] **05B-1:** Konflikt/unknown vysvětlí chybu ve zdroji, blokuje apply a
      souhrn/záznamy mají české labely i mobilní ekvivalent.
- [x] **05B-2:** Potvrzení shrne exact impact a zachová preview/version a
      idempotency pod povrchem.
- [x] **05C-1:** Source unavailable/auth expired/no changes/conflict/unknown/
      stale/ambiguous/offline a exact retry jsou testované; raw source data ani
      credentials nejsou v URL, telemetry nebo cache.
- [x] **05C-2:** Production dependency graph a request test prokážou, že route
      nevytváří `File`, `FormData` ani multipart request; CTA „Načíst ze
      SimpleShopu“ volá pouze serverový endpoint podle `ADR-015`.

**Účastníci akceptace**

- [x] **06A-1:** Search je POST/body, PII není v URL/history a read-only actor
      nevidí mutation controls.
- [x] **06B-1:** Picker kontrakt vyhledá bezpečný cílový ticket, minimalizuje
      PII a má no/ambiguous/stale problem větve.
- [x] **06C-1:** Každá dostupná akce má „Kdy použít“, lidský label, dopad,
      confirm a success/recovery větu; reassign/transfer nevyžaduje UUID a do
      uzavření `GAP-AUX-SUPPORT-ACTIONS-01` zůstává skryté.
- [x] **06D-1:** 401/403/offline/session expiry vymaže P3 list, selection,
...
- [x] **07C-1:** Full/stale/cancel/permission/offline a stránkování jsou
...
- [~] **12A-1:** Všechny routy projdou axe, keyboard-only a manuálním
  ...
  neprokazuje WCAG AA.
- [x] **12B-1:** Vizuální matice pokryje 320–1440 px, 1280 × 800, 200% zoom,
      dlouhou češtinu, reduced motion a žádný horizontální page scroll.
- [x] **12C-1:** Měření nad fixture §12.3 doloží route bundle delta, CLS,
...
- [x] **12D-1:** Security review doloží permission/event scope, P3 wipe,
...
- [~] **13A-1:** Produkční shell/context má lokálně jeden landmark strom,
  explicitní feature/capability gates a fail-closed route matici. Veřejný
  staging readiness je zelený a anonymní context správně vrací private/no-store
  `401`, ale staging běží starý release `bfead325…`; finální `[x]` vyžaduje
  deploy aktuálního `main` a schválený přihlášený UAT účet.
- [~] **13B-1:** Dashboard používá skutečné, permission-safe metriky a
  degraded worker/sync stav; bez targetu nevykreslí CTA. Lokální endpoint,
  produkční route a browser gate jsou hotové, zbývá společný staging gate
  `AUX-13A`.
- [~] **13C-1:** Core obsah používá produkční content port, human publication
  summary a neimportuje preview adapter/fixtures.
- [~] **13D-1:** Aktualizace vstupenek prochází source → preview → apply →
  report bez browserového souboru a se stejným exact-retry/idempotency
  invariantem; lokální kód/testy jsou hotové, zbývá společný staging gate
  `AUX-13A`.
- [~] **13E-1:** Legacy ticket účastníci projdou produkčním POST/no-store
  vyhledáním bez PII v URL/cache a block/reactivate má oddělené manage
  oprávnění, current-event guard, rate limit, audit a exact retry.
  Reassign/transfer zůstává fail-closed skryté; resend/recovery importovaných
  identit čeká na `P4-06`–`P4-09`, `BLOCKER-AUTH-01` a staging `AUX-13A`.
- [~] **13F-1:** Rezervace používají produkční session-first keyset
  stránkování, kanonický capacity write a oddělené storno; PII je
  smluvně maskované. Finální `[x]` čeká na staging auth/context E2E
  společného `AUX-13A`; `BLOCKER-RES-03` je v hlavním plánu uzavřený.
- [~] **13G-1:** Oznámení integruje event/jednu session, preview invalidation a
  `sent`/`already_sent` bez smyšleného delivery stavu.
- [~] **13H-1:** Tým načte produkční assignments, POST person search i
  pojmenované options; serverové permission/event/phase/scope guardy a
...
- [~] **13J-1:** Historie změn používá serverové event/category/action,
  PII-safe actor/outcome, time/request filtry, cursor pagination a redigované
  minimální DTO. Exhaustive typovaný registr pokrývá všechny produkční i
  podporované historické akce; zbývá společný staging gate `AUX-13A`.
- [~] **13K-1:** Core nastavení integruje produkční read/update, stale,
  archive a audit. Skrytý `supportMessage` server dovolí pouze beze změny a
  neukládá jej do audit diffu; jeho UI se dokončí až po uzavření gapu. Finální
  `[x]` čeká na staging `AUX-13A`.
...
| `GAP-AUX-SUPPORT-ACTIONS-01` | Jaký je přesný rozdíl, precondition a dopad `reassign` versus `transfer`? | Obě volby zůstávají skryté; rozhodnutí musí pokrýt držitele, oba tickety, přístup, rezervace a confirm/success/recovery; rezervační dopad z `RES-03` je autoritativní vstup `P4-09` |
...
| 3.6   | 2026-09-02 | `AUX-13I` připojil bounded export job list/cursor, ready-only download s current-event/expiry/auditem a rozšířil CSV ochranu                                                                           | Unit 604/604; worker 8/8; browser 1053/1053; `docs/evidence/admin-ux-aux-13i-exports.md`                   |
| 3.5   | 2026-09-02 | `AUX-13K` zpevnil core settings current-event/archive/support tamper guard a audit diff                                                                                                                | Unit 604/604; `docs/evidence/admin-ux-aux-13k-settings.md`                                                 |
| 3.4   | 2026-09-02 | `AUX-13H` připojil bounded role list, POST person search, pojmenované rozsahy a current-event/phase/scope guardy                                                                                       | Browser 1050/1050; `docs/evidence/admin-ux-aux-13h-roles.md`                                               |
| 3.3   | 2026-09-02 | `AUX-13B` připojil dashboard k bezpečným event-scoped agregacím a oddělenému worker/sync degraded stavu                                                                                                | Browser 1047/1047; `docs/evidence/admin-ux-aux-13b-dashboard.md`                                           |
| 3.2   | 2026-09-02 | Produkční integrační řezy synchronizovaly lokální route evidence, testy a staging dependency; žádný lokální výsledek se nevydává za podepsané UAT                                                      | `docs/evidence/`                                                                                           |
| 3.1   | 2026-09-02 | `AUX-13D` napojil SimpleShop preview na source-revalidated, reasoned a idempotentní participant apply/report; bez browserového souboru, ticket credentialu nebo automatického e-mailu                  | Unit 597/597; browser admin 192/192; PostgreSQL CI test; `docs/evidence/admin-ux-aux-13d-ticket-import.md` |
| 3.0   | 2026-09-02 | `AUX-13F`: produkční session-first keyset endpoint nahradil v novém UI dva omezené legacy ready; vrací maskované reference, kapacitu, waitlist a bounded rezervace a skládá další stránky bez duplicit | Unit 592/592; browser admin 192/192; PostgreSQL CI test; `docs/evidence/admin-ux-aux-13f-reservations.md`  |
| 2.9   | 2026-09-02 | `AUX-12C`: reprodukovatelný Chromium trace pokryl maxima audit 100, rezervace 100, obsah 50, ticket preview 500 a support 5; ticket preview po naměřených 128 ms dostal přístupné stránkování po 25    | CLS 0,03246; všechny finální interakce 0 ms; axe + keyboard; `docs/evidence/admin-ux-qa.md`                |
| 2.8   | 2026-09-02 | `AUX-13J`: auditní category/action/time/request/cursor filtry se aplikují před SQL limitem; limit+1 dává přesné pageInfo a endpoint vrací minimální redigovaný DTO řez                                 | Unit 586/586; browser 1044/1044; build; `docs/evidence/admin-ux-aux-13j-audit.md`                          |
| 2.7   | 2026-09-02 | `AUX-13G`: autorizovaný target options endpoint a bezpečné klientské načtení doplnily produkční event/one-session preview/send řez; staging shell E2E zůstává otevřený                                 | Unit 584/584; browser 1044/1044; build; `docs/evidence/admin-ux-aux-13g-announcements.md`                  |
| 2.6   | 2026-09-02 | `AUX-13C`: produkční obsah čte jediný admin shell context, používá pouze fetch port a sdílí cílovou prezentační vrstvu; staging shell E2E zůstává otevřený                                             | Unit 581/581; production mock-boundary build; `docs/evidence/admin-ux-aux-13c-content.md`                  |
| 2.5   | 2026-09-02 | `AUX-12`: reprodukovatelná 7-viewport cross-route matice, bundle budget, CLS/long-task audit a security review; fyzický screen reader a max-page trace zůstávají otevřené                              | 11 rout × 7; axe/keyboard/overflow; CLS 0.03246; interaction 0 ms; build boundary                          |
