...
Účastník otevře detail přednášky nebo naskenuje QR. Q&A QR vede na `/app/interakce/:sessionId`. Po přihlášení se vrátí na tentýž bezpečný odkaz. Server přijme dotaz pouze pro publikovanou podporovanou session, při zapnutém globálním i session přepínači a v intervalu **začátek včetně, konec vyjma**. Rozhoduje čas serveru. Nová publikace programu není kvůli přepínačům nutná.
...
| 5    | Před startem zkuste odeslat. Při startu obnovte stav.                                | Před startem zamítnuto; od startu přijímá. Polling se projeví nejpozději po dalším obnovení.                               |
...
| 10   | Na konci session zkuste nový submit.                                                 | Od přesného `endsAt` odmítnuto. Přesný retry již přijatého dotazu zůstane úspěšný.                                         |
