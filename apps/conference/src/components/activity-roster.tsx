import type { ActivityRosterResponse } from '@byzon/domain/contracts';
import { Card, StatePanel, StatusBadge } from '@byzon/ui';

export const ActivityRoster = ({
  data,
}: {
  readonly data: ActivityRosterResponse;
}) => (
  <main className="app-page">
    <header>
      <p className="eyebrow">Vedoucí aktivity</p>
      <h1 data-route-heading tabIndex={-1}>
        Přihlášení na moje aktivity
      </h1>
      <p className="lead">
        Read-only jméno, firma a stav rezervace pouze u přiřazených aktivit.
      </p>
    </header>
    {data.sessions.length === 0 ? (
      <StatePanel kind="empty" title="Nemáte přiřazenou aktivitu">
        <p>Organizátor zatím vašemu účtu nepřiřadil žádnou session.</p>
      </StatePanel>
    ) : (
      data.sessions.map((session) => (
        <Card key={session.sessionId}>
          <p className="activation-kicker">Kapacita {session.capacity}</p>
          <h2>{session.title}</h2>
          <time dateTime={session.startsAt}>
            {new Intl.DateTimeFormat('cs-CZ', {
              dateStyle: 'medium',
              timeStyle: 'short',
              timeZone: 'Europe/Prague',
            }).format(new Date(session.startsAt))}
          </time>
          <ul>
            {session.participants.map((participant) => (
              <li key={participant.reservationId}>
                <strong>{participant.displayName}</strong>{' '}
                <span>{participant.company ?? 'Firma neuvedena'}</span>{' '}
                <StatusBadge
                  tone={participant.state === 'reserved' ? 'success' : 'info'}
                >
                  {participant.state === 'reserved'
                    ? 'Rezervováno'
                    : 'Čekací listina'}
                </StatusBadge>
              </li>
            ))}
          </ul>
        </Card>
      ))
    )}
  </main>
);
